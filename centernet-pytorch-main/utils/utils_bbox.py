...
def pool_nms(heat, kernel = 3):
    pad = (kernel - 1) // 2
    # 最大池化和卷积操作不同，最大池化是在一个通道内进行的卷积窗口滑动，完成最大值池化操作
    hmax = nn.functional.max_pool2d(heat, (kernel, kernel), stride=1, padding=pad)
    #hmax == heat返回的矩阵元素为True或False
    keep = (hmax == heat).float()
    return heat * keep

def decode_bbox(pred_hms, pred_whs, pred_offsets, pred_angle, confidence, cuda):
    #-------------------------------------------------------------------------#
    ...
    for batch in range(b):
        ...
        pred_angle  = pred_angle[batch].permute(1, 2, 0).view([-1, 1])
        ...
        #-----------------------------------------------#
        #   取出得分筛选后对应的结果, pred_wh_mask等都为一维
        #-----------------------------------------------#
        pred_wh_mask        = pred_wh[mask]
        pred_offset_mask    = pred_offset[mask]
        pred_angle_mask     = pred_angle[mask]
        ...
        bboxes[:, [0, 2]] /= output_w
        bboxes[:, [1, 3]] /= output_h
        detect = torch.cat([bboxes, pred_angle_mask, torch.unsqueeze(class_conf[mask],-1), torch.unsqueeze(class_pred[mask], -1).float()], dim=-1)
...
def postprocess(prediction, need_nms, image_shape, input_shape, letterbox_image, nms_thres=0.4):
    ...
    #-------------------------------------------------------------------------#
    #   预测只用一张图片，只会进行一次，只会在预测的时候进行第二次非极大值预测
    #-------------------------------------------------------------------------#
    for i, image_pred in enumerate(prediction):
        ...
        for c in unique_labels:
            ...
            #   获得某一类得分筛选后全部的预测结果
            #------------------------------------------#
            detections_class = detections[detections[:, -1] == c]  #该步将所有的类别为c的预测结果拿出来，做非极大抑制操作
            if need_nms:
                #------------------------------------------#
                #   ...................................！
                #------------------------------------------#
