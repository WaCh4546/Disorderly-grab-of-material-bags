...
if __name__ == "__main__":
    ...
    if model_path != '':
        #-------------------------------------------------------------------------------------------#
        #   model.state_dict()可获取网络超参数，训练的时候为防止出错，加载预训练权重过程比较复杂，预测则简单
        #-------------------------------------------------------------------------------------------#
    ...
    #  model.train()代表网络开始训练，会使用梯度下降法来更新权值，但model.train()的返回值仍为self
    ...
    if True:
        ...
        # 构建了一个Adam优化器来更新网络参数，设置了学习率lr和L2惩罚项weight_decay
        # 优化器的主要作用就是根据损失函数求出的loss和损失函数梯度对神经网络的参数进行更新
        optimizer       = optim.Adam(model_train.parameters(), lr, weight_decay = 5e-4)
        # 可借助optim.lr_scheduler类调整学习率，optimizer为要更改学习率的优化器；step_size为每训练step_size个epoch，
        # 更新一次参数，gamma为更新lr的乘法因子；
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            lr_scheduler.step()  # 该语句写在此处用来更新学习率，但是每隔几次epoch更新一次学习率参数，是由step_size决定的。
    ...
    if True:
        ...
        for epoch in range(start_epoch, end_epoch):
            fit_one_epoch(model_train, model, loss_history, optimizer, epoch, 
                    epoch_step, epoch_step_val, gen, gen_val, end_epoch, Cuda, backbone)
            lr_scheduler.step()
        torch.save(model.state_dict(),'logs/2022_0706final_net.pth')