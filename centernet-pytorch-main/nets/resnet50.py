...
class Bottleneck(nn.Module):
    # 类内直接定义属性，该属性是属于这个类的，类似于静态成员变量可以直接通过类名.变量名调用，
    # 而self.属性名是属于类对象的，只有实例化以后才可以调用
    expansion = 4
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        # 当步长不等于1会导致输入输出维度不相等，当self.inplanes != planes * block.expansion
        # 也说明维度不相等，在特征图相加之前要保证维度不变，故要进行if语句中的处理
