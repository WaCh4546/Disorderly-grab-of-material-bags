#---------------------------------------------------------------------#
#   1、将单张图片预测、摄像头检测和FPS测试功能
#   2、整合到了一个py文件中，通过指定mode进行模式的修改。
#   3、理论上应该使用测试集的图片来判断网络的性能，因为训练集用于训练网络，
#   验证集也参与了超参数的训练过程，不能使用训练集和验证集图片来测试网络性能
#---------------------------------------------------------------------#
...
if __name__ == "__main__":
    centernet = CenterNet()
    # for name in centernet.state_dict():
        # print(name)
        # print(centernet.state_dict()[name])
